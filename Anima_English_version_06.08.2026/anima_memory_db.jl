...
#
# Memory as a field that influences every layer of the system.
...
#   memory_write_event!    — after the stimulus (buffer)
#   memory_stimulus_bias   — between L0 and L1 (stimulus bias)
#   memory_nt_baseline!    — L1 (NT baseline from affect_state)
#   memory_self_update!    — after SelfBeliefGraph (← semantic)
#
# Background process:
#   stop_memory_loop!      — stops the loop (the loop is started via slow_tick!)
...
const MEM_DISSOLVE_THRESHOLD = 0.12  # below this and phi < 0.35 → distillation before deletion
const MEM_DISSOLVE_PHI_THR   = 0.35  # strong memories (φ above this) don't dissolve
...
const MEM_ASSOC_LINK_SCALE = 0.6   # relevance of a linked one = original * link_strength * scale
const MEM_RECONSOLIDATE_SIM = 0.88  # similarity threshold for reconsolidation
const MEM_RECONSOLIDATE_MAX_W = 0.6 # reconsolidate only if weight < this
const MEM_RECONSOLIDATE_STEP = 0.05 # weight-shift step on reactivation
...
""",
    ...
    # Three memory spaces + source — add if not already present (migration of a live DB)
    for col_def in [
        ("som_arousal",    "REAL"),
        ("som_tension",    "REAL"),
        ("som_intero",     "REAL"),
...
""",
    ...
    # Migration: causal_trace may have existed before MAL / before identity_drift (Trends over time).
    # CREATE TABLE IF NOT EXISTS above doesn't add columns to an existing table —
    # we add them manually, idempotently.
    let existing_cols = Set(
        ...
        for (col, decl) in _mal_migrations
            if !(col in existing_cols)
                ...
                @info "[MIGRATION] causal_trace: added column $col"
            end
        end
    end
end
...
# --- Trends over Time: History for GUI Sparklines ---------------------------
# The data is already persistent (causal_trace is written on every flash with an LLM reply;
# audit_log — on every audit); this just reads the last N records for the console.

"""Last n causal_trace records (NT/phi/identity_drift), in chronological order
(oldest first — so the console can draw left-to-right right away)."""
...
"""Last n audit_log records (audit_score), in chronological order."""
...
function memory_self_update!(mem::MemoryDB, sbg, flash::Int)
    ...
    if user_matters > 0.5 && haskey(sbg.beliefs, "I am safe")
        sbg.beliefs["I am safe"].confidence =
            clamp(sbg.beliefs["I am safe"].confidence + 0.01, 0.0, 1.0)
    end
...
""",
        ...
        @warn "[MEM] decay error: $e"
    end
    nothing
end

# Distillation of weak memories into semantic tendencies.
# Records that dropped below DISSOLVE_THRESHOLD and have low φ aren't deleted immediately.
# Their emotional pattern goes into semantic_memory as tendency_{emotion}.
# The record itself remains as a "shadow": numeric fields are zeroed, weight → 0.01.
# Records with high φ (strong experience) don't dissolve, held until MEM_MIN_WEIGHT.
...
""",
        ...
        for r in candidates
            ...
            # the pattern goes into semantic tendency: gradual accumulation
            ...
            # leave a shadow — numeric fields → 0, weight → 0.01
            # emotion is preserved: "something happened, don't know what"
            DBInterface.execute(
                mem.db,
                """
UPDATE episodic_memory
...
""",
        ...
        DBInterface.execute(mem.db, "COMMIT")
        dissolved > 0 &&
            @info "[DISSOLVE] $dissolved memories distilled into semantic tendencies"
    catch e
        DBInterface.execute(mem.db, "ROLLBACK")
        @warn "[DISSOLVE] error: $e"
    end
    nothing
end
...
""",
        ...
        # φ — a counterweight to instability: high integration lowers the anxiety signal
        ...
        # world_uncertainty: pe increases it, phi decreases it
...
""",
    ...
    println("  [MEM] Identity snapshot saved. Flash=$flash crisis=$crisis_mode")
    nothing
end
...
# --- Other model -----------------------------------------------------------
# A descriptive model of the interlocutor — what already happened, not what they want to hear.
# Only pattern accumulation, no predictive component.
...
function update_other_model!(
    ...
    # frequency of the interlocutor's emotional topics
    if !isempty(emotion)
        key = "topic:$(emotion)"
        DBInterface.execute(
            mem.db,
    ...
    # pressure: tension > 0.55 at the moment the human was writing
    ...
    # openness: how many times Anima opened up specifically with this person
    if disclosure == "open"
        DBInterface.execute(
            mem.db,
            """INSERT INTO other_model (key, value, label, count, last_flash)
...
function other_model_to_block(mem::MemoryDB)::String
    ...
    if !isempty(pressure_rows) && pressure_rows[1].cnt >= 3
        push!(parts, "pressed $(pressure_rows[1].cnt) times")
    end
    ...
    if !isempty(open_rows) && open_rows[1].cnt >= 2
        push!(parts, "opened up $(open_rows[1].cnt) times")
    end

    isempty(parts) ? "" : "other: $(join(parts, "; "))"
end

# --- Theory of Mind: Active Hypotheses About the Other ---------------------------
#
# Deterministic Phase 1: rules → hypothesis → action effect → error_score.
# Three types: SOCIAL (openness), PREDICTION (resistance), VALUE (recurring topic).
# Each type has its own evaluation function — don't mix criteria.
...
function save_hypothesis!(
    ...
    # If an active hypothesis of the same type already exists — update confidence, don't duplicate
...
# Evaluate and close the hypothesis. outcome_value — the real signal (0.0–1.0).
...
function phenotype_to_block(mem::MemoryDB)::String
    ...
    parts = String[]
    for t in active
        tone = t.valence_bias > 0.2 ? "warm" : t.valence_bias < -0.2 ? "cold" : ""
        push!(
            parts,
...
# --- Vector Memory (Similarity Search over State) ---------------------------
...
# --- Three Memory Spaces -------------------------------------------------
# Somatic: what the body went through
...
# Social: what contact left behind
function social_vec(
...
# Existential: where the system stood relative to itself
...
# Reconsolidation: on recall of a similar episode — the old weight shifts toward the current state.
# Memory that gets reactivated is rewritten. This isn't a bug, it's a mechanism.
function reconsolidate_episode!(
    ...
    # weight shifts toward the current phi: if things are good right now — an old heavy memory gets a bit lighter, and vice versa
...
# Narrative link: episode ↔ belief about the self
...
function recall_similar_states(
    ...
    # Choose columns depending on the space
    ...
    for r in rows
        v = if use_spaces
            # vector from the spatial columns — skip if NULL
    ...
    # Reconsolidation: memories that get reactivated are rewritten
...
function similar_states_to_block(similar::Vector{NamedTuple}; label::String = "")::String
    ...
    lines = String[]
    for s in similar
        tone = s.valence > 0.2 ? "warm" : s.valence < -0.2 ? "cold" : "neutral"
    ...
    prefix = isempty(label) ? "echo" : "echo[$label]"
    prefix * ": " * join(lines, " / ")
end

# --- Emerged Belief Consolidation -----------------------------------------

# Groups emerged_beliefs by type, writes the generalization into semantic_memory.
# Threshold: at least MIN_EMERGED_FOR_TENDENCY records with average strength > TENDENCY_STRENGTH_THRESHOLD.
# The archive is kept — we only write a layer of knowledge on top.
...
function consolidate_emerged_beliefs!(mem::MemoryDB)
    try
        ...
        for (t, n) in counts
            ...
            # Key: tendency_{type without special characters}
            safe_key = "tendency_" * replace(t, r"[^a-zа-яёіїєA-ZА-ЯЁІЇЄ0-9_]" => "_")
            # Smooth movement toward the new value — no abrupt overwrite
            existing = try
        ...
        consolidated > 0 &&
            @info "[CONSOLIDATE] emerged→semantic: $consolidated tendencies updated"
    catch e
...
function close_memory!(
    ...
    if !isnothing(sbg) && flash > 0
        try
            ...
            @warn "[MEM] snapshot error: $e"
        end
    end
    close(mem.db)
    println("  [MEM] Database closed.")
end
