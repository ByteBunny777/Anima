# anima_gui_bridge.jl
# Mirrors already-computed values into files alongside what's already printed
# via println/@info. No new logic here — just serialization.
...
# Keep audit and llm meta between flashes — so a new flash doesn't zero out the previous audit
...
function write_gui_state!(a::Anima, r; audit = nothing, cf_co = nothing)
    ...
    # update the cache if there are new values
    if !isnothing(audit)
    ...
    causal = Dict{String,Any}(
        ...
        # from the cache — always the last known value
...
function push_gui_event!(kind::String, payload::Dict)
    # llm_request — cache model and size separately
    if kind == "llm_request"
...
function gui_reset_session!()
    # Clear chat and events on every new run
    # gui_state.json isn't touched — it's simply overwritten on the first flash
    lock(GUI_JSONL_LOCK) do
...
# Live state — read directly from Anima between flashes, without writing to a file
