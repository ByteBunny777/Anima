...
# NOTE: previously this rotated between ready-made sentences (flash%3) — the
# same written phrase every time for one level range. Now returns a raw state
# vector; verbalization (tone, wording) is done by the LLM during generation,
# not by pre-written text. If the result reads too flat to the LLM, that
# means the system prompt needs an added explanation of how to read these
# [signal: ...] tags, not that text should be put back here.
function shame_note(sm::ShameModule, flash::Int = 0)::String
    sm.level < 0.3 && sm.chronic < 0.4 && return ""
    "[signal: shame level=$(round(sm.level, digits=2)) chronic=$(round(sm.chronic, digits=2)) internalized_gaze=$(round(sm.internalized_gaze, digits=2))]"
end
...
# --- Epistemic Defense ----------------------------------------------------
# NOTE: bias description dicts (EP_DESC/EP_DISTORT) removed — computed but
# never read anywhere (not in the prompt, not in the GUI). The bias-selection
# logic itself stays — it's genuinely causal (rule-based on
# dissonance/shame/fatigue/moral_agency).
...
# NOTE: each entry used to also carry a ready-made phrase — computed but
# never read (not in the prompt, not in the GUI). Removed; only the type
# remains, which is genuinely used below via SYMPTOM_DIR.
const SYMPTOM_MAP=Dict(
    ("Anger", "repression") => "anger_as_depression",
    ("Anger", "denial") => "anger_as_passive_aggr",
    ("Fear", "rationalization")=>"fear_as_control",
    ("Fear", "suppression") => "fear_as_numbness",
    ("Sadness", "denial") => "grief_as_numbness",
    ("Sadness", "displacement")=>"grief_as_irritability",
    ("Joy", "suppression")=>"love_as_hostility",
    ("Disgust", "projection") => "projection_as_contempt",
)
# NOTE: previously fixed magnitudes here — the same number every time,
# regardless of how strongly the symptom actually manifested. Kept only the
# DIRECTION (which reactor, which way) — a psychologically grounded
# structure, not an invention. Magnitude is now computed in generate_symptom!
# from live signals (attention narrowing, prediction error).
...
function generate_symptom!(
    ...
    # distortion: narrowed attention (low radius) and high prediction error
    # genuinely amplify how strongly the symptom shows — not just a counter
